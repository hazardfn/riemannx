defmodule RiemannxTest.BatchQueue do
  ...
  test "Batch queue tests" do
    ...
    push_fun = fn e, q -> BatchQueue.push(q, e) end

    ## Fresh queue
    q0 = BatchQueue.new()
    assert false == BatchQueue.batch_available?(q0)
    assert {:ok, {q0, []}} == BatchQueue.get_batch(q0)

    ## Queue with events
    q1 = Enum.reduce(events1, q0, push_fun)
    assert true == BatchQueue.batch_available?(q1)
    {:ok, {q2, batch}} = BatchQueue.get_batch(q1)
    assert batch == batch1

    ## Add more events and test second batch
    q3 = Enum.reduce(events1, q2, push_fun)
    {:ok, {_q4, batch}} = BatchQueue.get_batch(q3)
    assert batch == batch2
  end
end
